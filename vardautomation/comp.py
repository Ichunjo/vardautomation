...
class Comparison:
    ...
    def __init__(
        ...
        picture_type: PictureType | list[PictureType] | None = None,
    ) -> None:
        """
        ...
        :param frames:              Additionnal frame numbers that will be added
                                    to the total of ``num``, defaults to None
        :param picture_type:        Select picture types to pick, default to None
        """
        self.clips = clips
        self.path = VPath(path)
        self.path_diff: VPath | None = None

        # Check length of all clips
        lens = {c.num_frames for c in clips.values()}
        if len(lens) != 1:
    ...
    def extract(self, writer: Writer = Writer.PYTHON, compression: int = -1, force_bt709: bool = False) -> None:
        ...
        :param compression:         Compression level. It depends of the writer used,
                                    defaults to -1 (auto)
        ...
        for name, clip in self.clips.items():
            ...
            clip = clip.resize.Bicubic(
                format=vs.RGB24,
                matrix_in=vs.MATRIX_BT709 if force_bt709 else None,
                dither_type=DitherType.ERROR_DIFFUSION,
            )

            pad = len(str(self.max_num))
            path_images = [path_name / f"{name}_{str(f).zfill(pad)}.png" for f in self.frames]
            ...
            # imwri lib is slower even asynchronously requested
                ...
                clip = select_frames(clip, self.frames)
                clip = clip.std.ModifyFrame(
                    clip, lambda n, f, path_images=path_images: _saver(writer, compression)(n, f, path_images)
                )
                with open(os.devnull, "wb") as devnull:
                    clip.output(devnull, y4m=False, progress_update=_progress_update_func)
                logger.logger.opt(raw=True).info("\n")
    ...
    def magick_compare(self) -> None:
        ...
        pad = len(str(self.max_num))
        cmds = [
            f'magick compare "{i1.to_str()}" "{i2.to_str()}" "{self.path_diff.to_str()}/diff_{str(f).zfill(pad)}.png"'
    ...
    def upload_to_slowpics(self, config: SlowPicsConf) -> None:
        ...
        for i, (name, images) in enumerate(
            zip(list(self.clips.keys()) + (["diff"] if self.path_diff else []), all_images)
        ):
            ...
            while True:
                ...
                if all(
                    ...
                    for f in vs.core.std.Splice(
                        [select_frames(c, [rnum]) for c in self.clips.values()], mismatch=True
                    ).frames()
                ):
                    break
                _attempts += 1
                ...
                if _attempts > _MAX_ATTEMPTS_PER_PICTURE_TYPE:
                    logger.warning(
                        f"{self.__class__.__name__}: {_MAX_ATTEMPTS_PER_PICTURE_TYPE} attempts were made "
                        f"for sample {len(samples)} and no match found for {picture_types}; stopping iteration..."
                    )
                    break
...
def make_comps(
    ...
    frames: Iterable[int] | None = None,
    *,
    picture_types: PictureType | list[PictureType] | None = None,
    force_bt709: bool = False,
    writer: Writer = Writer.PYTHON,
    compression: int = -1,
    magick_compare: bool = False,
    slowpics_conf: SlowPicsConf | None = None,
) -> None:
    """
    ...
    :param force_bt709:         Force BT709 matrix before conversion to RGB24, defaults to False
    :param writer:              Writer method to be used, defaults to Writer.PYTHON
    :param compression:         Compression level. It depends of the writer used, defaults to -1 (auto)
    ...
    :param slowpics_conf:       slow.pics configuration. If specified, images will be uploaded accordingly
    """
    ...
    if writer == Writer.PYTHON:
        ...
        def _write_png(buf: bytes, width: int, height: int) -> bytes:
            ...
            def _png_pack(png_tag: bytes, data: bytes) -> bytes:
                ...
                return (
                    struct.pack("!L", len(data)) + chunk_head + struct.pack("!L", 0xFFFFFFFF & zlib.crc32(chunk_head))
                )
