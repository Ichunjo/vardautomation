...
class Chapters(ABC):
    ...
    def set_names(self, names: Sequence[str | None]) -> None:
        ...
        :param names:           List of optional names. A ``None`` value won't change
                                the name of the current chapter list
        """
    ...
    @abstractmethod
    def to_chapters(self, fps: Fraction, lang: Lang | None) -> list[Chapter]:
        """
        Convert the Chapters object to a list of chapter

        :param fps:             Framerate Per Second
        :param lang:            Language of the chapter. If specified it will override
                                the current language of the Chapters object
        """
...
class OGMChapters(Chapters):
    ...
    def create(self, chapters: list[Chapter], fps: Fraction) -> None:
        ...
        with self.chapter_file.open("w", encoding="utf-8") as file:
            for i, chapter in enumerate(chapters, start=1):
                file.writelines(
                    [
                        f"CHAPTER{i:02.0f}={Convert.f2ts(chapter.start_frame, fps)}\n",
    ...
    def shift_times(self, frames: int, fps: Fraction) -> None:
        ...
        newchaptimes = [
            f"{k}={Convert.seconds2ts(max(0, Convert.ts2seconds(v) + shifttime))}\n"
            for chaptime in chaptimes
            for k, v in [chaptime.split("=", 1)]
        ]
...
class MplsReader:
    ...
    def get_playlist(self) -> list[MplsFile]:
        ...
        return [
            self.MplsFile(mpls_file=mpls_file, mpls_chapters=self.parse_mpls(mpls_file)) for mpls_file in mpls_files
        ]

    def write_playlist(
        self, output_folder: AnyPath | None = None, *, chapters_obj: type[Chapters] = MatroskaXMLChapters
    ) -> None:
        """
    ...
    def parse_mpls(self, mpls_file: AnyPath) -> list[MplsChapters]:
        ...
        for i, playitem in enumerate(playlist.play_items):
            ...
            # Add the m2ts name
            if (name := playitem.clip_information_filename) and (ext := playitem.clip_codec_identifier):
...
class IfoReader:
    ...
    def write_programs(
        self,
        output_folder: AnyPath | None = None,
        *,
        chapters_obj: type[Chapters] = MatroskaXMLChapters,
        ifo_file: str = "VTS_01_0.IFO",
    ) -> None:
        """
    ...
    def _ifochapters_to_chapters(self, pb_times: list[vts_ifo.vts_pgci.PlaybackTime], fps: Fraction) -> list[Chapter]:
        raw_fps = fps.numerator if (fpsnum := int(fps.numerator / 1000)) == 0 else fpsnum
        ...
        pb_frames = [pb_frame + sum(pb_frames[: -(len(pb_frames) - i)]) for i, pb_frame in enumerate(pb_frames[:-1])]

        return [
            Chapter(
                name=f"{self.default_chap_name} {i:02.0f}", start_frame=pb_frame, end_frame=pb_frames[i], lang=self.lang
            )
