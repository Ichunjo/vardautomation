...
__all__ = ["WaveHeader", "audio_async_render", "clip_async_render"]
...
def _finish_frame_video(frame: vs.VideoFrame, outfile: BinaryIO) -> None:
    outfile.write(b"FRAME\n")
    outfile.writelines(frame)  # type: ignore[call-overload]
...
def audio_async_render(
    ...
    progress: str | None = "Rendering audio...",
) -> None:
    """
...
def _wav_header(audio: vs.AudioNode, bps: int, block_align: int, data_size: int) -> bytes:
    ...
    fmt_chunk_data = struct.pack(
        "<HHIIHH", format_tag, audio.num_channels, audio.sample_rate, bps, block_align, audio.bits_per_sample
    )
